...
class Paster {
    ...
    public static paste() {
        ...
        function padFromStart(num: string, n: number): string {
        ...
        this.getImagePath(filePath, selectText, this.folderPathConfig, this.showFilePathConfirmInputBox, this.filePathConfirmInputBoxMode, function (err, imagePath) {
            try {
                ...
                if (fs.existsSync(_i)) {
                    Logger.showInformationMessage(`File ${_i} existed.Would you want to replace?`, 'Replace', 'Cancel').then(choose => {
                        if (choose != 'Replace') return;

                        instance.saveAndPaste(editor, _i);
                    });
                } else {
                    instance.saveAndPaste(editor, _i);
                }
